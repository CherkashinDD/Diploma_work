...
На домашней странице находится основное меню(основное меню отображается абсолютно на всех страницах), через которое можно попасть на все основные вкладки: Портфолио, Услуги, Контакты. Так же логотип является кнопкой для возвращения на главную страницу.

Портфолио.

На странице портфолио имеется возможность ознакомиться с основными успешными проектами фирмы. В нижней части страницы находится меню выбора категорий работ фирмы: Архитектура, Благоустройство, Коммерческие интерьеры, Жилые интерьеры. При переходе на каждую страницу, можно ознакомиться с работами. На данном этапе разработки сайта, владельцем фирмы предоставлено ограниченное количество материала, в связи с чем работы имеют вид картинок-ссылок с «заглушками». В будущем при переходе по каждой ссылке можно будет подробно ознакомиться с каждой работой.
 
Услуги.

Данная страница знакомит с услугами, оказываемыми фирмой.

Контакты.

На данной странице расположены контактные данные фирмы.
...
Для настройки использовался файл setting.py. В данном файле было произведено подключение приложения cherepaha, путём добавления 'cherepaha' в список INSTALLED_APPS. Для того что бы Django видел шаблоны, в словарь TEMPLATES добавлен ключ со значением – 'DIRS: [BASE_DIR/'templates']. Для управления статистическими файлами использовались следующие настройки:
STATIC_URL = '/static/'
STATICFILES_ DIR = [BASE_DIR/'cherepaha/static']
	Для реализации логики отображения HTTP-запросов и формирования ответ используется файл views.py. Для данного сайта достаточно импортировать функцию render из модуля  django.shortcuts. Данная функция упрощает процесс создания HTTP-ответа от приложения. Она сочетает в себе получение HTML-шаблона и данных для его рендеринга.
	Для создания путей, в директории cherepaha создан файл urls.py. Данный файл создан отдельно, для отражения возможностей Django. Создание отдельного файла делает управление URL-маршрутизацией более удобным и организованным. В основной файл urls.py маршруты включаются при помощи функции include().
	В директории ststic размещены 2 папки css и img. Директория css содержит файлы style.css и style2.css, в которых заданы стили для элементов страниц. Директория img содержит картинки для отображения на сайте.
В директории templates находятся HTML-шаблоны. 
 
Виртуальное окружение (venv) содержит следующие библиотеки:
...
Учитывая то, что основа у проекта одна начну со схемы.
...
└── venv/
```
В основной директории содержится main.py. Это основной файл для запуска приложения. В нём, для создания экземпляра приложения импортируется микро-фреймворк Flask. Для создания экземпляра Flask-приложения, сохраняем его в переменную app (app=Flask(__name__)). Это позволит зарегистрировать Blueprint. В моём случае их будет два:
...
В директории routers (для удобства) расположены пути. В файле main_menu находятся ссылки основного меню. В файле portfolio_menu находятся ссылки меню на странице Портфолио. Файлы содержат функции, обвёрнутые в декораторы, определяющие маршруты для обработки запросов. Для этого необходимо импортировать из Flask два модуля: Blueprint и render_template. В переменные main_menu_routes и portfolio_menu_routes сохраняем экземпляр класса Blueprint, передавая в него в качестве аргумента его имя (которое мы прописываем потом в url_perfix зарегистрированного Blueprint в файле main.py) и имя модуля (__name__). В свою очередь render_template необходим для того, что бы рендерить шаблоны HTML и возвращать их в качестве ответа на запрос.
Пример из проекта:

![image](https://github.com/user-attachments/assets/50e78d84-d243-4229-82cc-957d67dc51da)

Виртуальное окружение (venv) содержит библиотеки, установленные по умолчанию вместе с Flask:
blinker==1.9.0
click==8.1.7
colorama==0.4.6
Flask==3.1.0
...
└── venv/  
``` 
Как видно схема идентична схеме приложения с использованием Flask. Однако, для реализации приложения по мимо FastAPI понадобятся следующие библиотеки: Uvicorn (для запуска сервера, который позволяет запускать асинхронные Python web-приложения), Jinja2(шаблонизатор для генерации HTML).  
В файл main.py импортируется класс FastAPI, который является основным компонентом для создания web-приложения. Для определения маршрутов, обработки запросов и настройки приложения создаём экземпляр класса FastAPI:

app=FastAPI().

Для обработки статистических файлов понадобится StaticFilse и mount, которые нужно предварительно импортировать. Выглядит это следующим образом:
	
app.mount("/static", StaticFiles(directory="static"), name="static")

Остаётся добавить маршруты, которые заранее определены в других модулях, воспользовавшись методом app.include_router, что позволяет разделить приложение на более мелкие и управляемые части.
	В директории router расположены 2 файла: main_menu и portfolio_menu. По аналогии с предыдущим приложением, в них находятся ссылки для основного меню и меню портфолио. Для реализации кода понадобится произвести следующий импорт:
 
![image](https://github.com/user-attachments/assets/d0f7bbaa-ff60-4879-8e24-2f64df9eb7e6)

Для определения маршрутов  необходимо создать экземпляр APIRouter()
Request – это объект, который представляет HTTP-запрос.
	Environment () управляет настройками Jinja2, FileSystemLoaderа необходим для загрузки шаблонов. 
Template необходим для чтения шаблонов.
HTMLResponse используется для возврата HTML-кода в ответ на HTTP-запрос.

![image](https://github.com/user-attachments/assets/f1427c25-cd45-4dcc-bcb2-6d44d3e489c2)

Как известно, FastAPI поддерживает асинхронное программирование. Но в данном проекте нет смысла его использовать,  так как он очень прост.
